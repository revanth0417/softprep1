	...
	if (!(window.MathJax && MathJax.Hub)) {
			...
			b.Object = c({
				...
				Augment : function(f, g) {
					...
					if (f != null) {
						...
						if (f.toString !== this.prototype.toString
								&& f.toString !== {}.toString) {
		...
		(function(BASENAME) {
			...
			var CALLBACK = function(data) {
				var cb = function() {
					return arguments.callee.execute.apply(arguments.callee,
							arguments)
				};
			...
			CALLBACK.prototype = {
				...
				execute : function() {
					if (!this.called || this.autoReset) {
						...
						return this.hook.apply(this.object, this.data
			...
			var TESTEVAL = function() {
				...
				if (window.__TeSt_VaR__) {
					...
					if (window.execScript) {
						EVAL = function(code) {
							BASE.__code = code;
							code = "try {" + BASENAME + ".__result = eval("
									+ BASENAME + ".__code)} catch(err) {"
		...
		(function(e) {
			...
			a.Ajax = {
				...
				fileURL : function(j) {
					...
					if (i && i[1] in b) {
						j = (b[i[1]] || this.config.root)
								+ j.substr(i[1].length + 2)
					}
					return j
				},
				fileName : function(j) {
				...
				timer : {
					...
					execute : function() {
						this.hook.call(this.object, this, this.data[0],
								this.data[1])
					},
					...
					checkLength : function(i, l, n) {
						...
						try {
							...
							if (k.message
									.match(/protected variable|restricted URI/)) {
								m = 1
							} else {
				...
				Styles : function(k, l) {
					...
					if (i === "") {
						...
						if (j.styleSheet
								&& typeof (j.styleSheet.cssText) !== "undefined") {
				...
				if (d) {
					...
					for (var a = 0; a < d.length; a++) {
						if (d[a] instanceof Array) {
							f.appendChild(this.Element(d[a][0], d[a][1],
									d[a][2]))
						} else {
							if (c === "script") {
		...
		MathJax.Localization = {
			...
			_ : function(b, a) {
				...
				return this.processString(this.lookupPhrase(b, a), [].slice
						.call(arguments, 2))
			},
				...
				for (var d = 0, b = e.length; d < b; d++) {
					if (e[d] instanceof Array) {
						...
						if (typeof f[1] === "string") {
							...
							c = c
									.concat(this.processMarkdown(a, f.slice(2),
											g))
						} else {
							if (f[1] instanceof Array) {
								c = c
										.concat(this.processSnippet.apply(this,
												f))
							} else {
								if (f.length >= 3) {
									c.push([ f[0], f[1],
											this.processSnippet(g, f[2]) ])
								} else {
									c.push(e[d])
								}
							}
			...
			processMarkdown : function(b, h, d) {
				...
				for (var f = 1, a = c.length; f < a; f += 8) {
					if (c[f + 1]) {
						...
						if (c[f + 3]) {
							e = this.processString(c[f + 4].replace(/^\s/, "")
									.replace(/\s$/, ""), h, d);
							if (!(e instanceof Array)) {
								e = [ e ]
							}
			...
			loadFile : function(b, d, e) {
				...
				if (!b.match(/^([a-z]+:|\[MathJax\])/)) {
					var a = (this.strings[this.locale].directory
							|| this.directory + "/" + this.locale || "[MathJax]/localization/"
							+ this.locale);
					b = a + "/" + b
				}
			...
			loadDomain : function(c, e) {
				...
				if (a) {
					if (!a.isLoaded) {
						...
						if (b) {
							return MathJax.Callback.Queue(b,
									[ "loadDomain", this, c ]).Push(e || {})
						}
					}
		...
		MathJax.Message = {
			...
			Init : function(a) {
				...
				if (!this.div) {
					...
					this.text = this.div.appendChild(document
							.createTextNode(""))
				}
				return true
			},
			...
			MoveFrame : function() {
				var a = (MathJax.Message.quirks ? document.body
				...
				if (MathJax.Hub.config.messageStyle === "simple") {
					if (b === "LoadFile") {
						...
						if (b === "ProcessMath") {
							...
							if (b === "TypesetMath") {
								if (!this.typesetting) {
									this.typesetting = this
											.localize("Typesetting")
											+ " "
								}
								a = this.typesetting;
								this.typesetting += "."
			...
			Set : function(c, e, b) {
				...
				if (c instanceof Array) {
					...
					try {
						c = MathJax.Localization._.apply(MathJax.Localization,
								c)
					} catch (a) {
						if (!a.restart) {
							throw a
						...
						if (!a.restart.called) {
							...
							MathJax.Callback.After([ "Set", this, c, e, b ],
									a.restart);
							return e
						}
					}
					...
					if (this.text) {
						...
						if (this.current == null) {
							...
							if (a === 0) {
								...
								this.timer = setTimeout(MathJax.Callback([
										"Remove", this ]), a)
							}
						} else {
		...
		MathJax.Hub = {
			...
			getJaxByInputType : function(f, e) {
				var c = [], b = this.elementScripts(e);
				for (var d = 0, a = b.length; d < a; d++) {
					if (b[d].MathJax && b[d].MathJax.elementJax && b[d].type
			...
			isJax : function(a) {
				...
				if (a && a.tagName != null
						&& a.tagName.toLowerCase() === "script") {
					if (a.MathJax) {
						return (a.MathJax.state === MathJax.ElementJax.STATE.PROCESSED ? 1
								: -1)
					}
					if (a.type
							&& this.inputJax[a.type.replace(/ *;(.|\s)*/, "")]) {
						return -1
					}
				}
				return 0
			...
			setRenderer : function(d, c) {
				...
				if (!MathJax.OutputJax[d]) {
					...
					return MathJax.Ajax.Require(b,
							[ "setRenderer", this, d, c ])
				} else {
			...
			Typeset : function(c, d) {
				...
				if (b.count) {
					var a = MathJax.Callback.Queue([ "PreProcess", this,
							b.elements ], [ "Process", this, b.elements ])
				}
			...
			PreProcess : function(e, g) {
				...
				if (c.count) {
					...
					b.Push([ "Post", this.signal,
							[ "Begin PreProcess", c.elements ] ]);
			...
			takeAction : function(g, d, h) {
				...
				if (c.count) {
					var b = [ "Delay", MathJax.Callback,
							this.processSectionDelay ];
					if (!b[2]) {
						b = {}
					}
					a.Push([ "Post", this.signal, [ "Begin " + g, f ] ], [
							"Post", this.signal, [ "Begin Math", f, g ] ], [
							"prepareScripts", this, g, f, e ], [ "Post",
							this.signal, [ "Begin Math Input", f, g ] ], [
							"processInput", this, e ], [ "Post", this.signal,
							[ "End Math Input", f, g ] ], b, [ "prepareOutput",
							this, e, "preProcess" ], b, [ "Post", this.signal,
							[ "Begin Math Output", f, g ] ], [ "processOutput",
							this, e ], [ "Post", this.signal,
							[ "End Math Output", f, g ] ], b, [
							"prepareOutput", this, e, "postProcess" ], b, [
			...
			prepareScripts : function(h, e, g) {
				...
				for (var d = 0, a = b.length; d < a; d++) {
					var c = b[d];
					if (c.type
							&& this.inputJax[c.type.replace(/ *;(.|\n)*/, "")]) {
						if (c.MathJax) {
							if (c.MathJax.elementJax
									&& c.MathJax.elementJax.hover) {
								MathJax.Extension.MathEvents.Hover
										.ClearHover(c.MathJax.elementJax)
							}
			...
			processInput : function(a) {
				...
				try {
					while (a.i < d) {
						...
						if (!h.MathJax.elementJax
								|| h.MathJax.state === i.UPDATE) {
						...
						if (c - a.start > this.processUpdateTime
				...
				while (c.j < c.jaxIDs.length) {
					...
					if (d[f]) {
						try {
							...
							return MathJax.Callback.After([ "prepareOutput",
									this, c, f ], b.restart)
						}
					}
					c.j++
			...
			processOutput : function(h) {
				...
				try {
					while (h.i < a) {
						...
						if (e - h.start > this.processUpdateTime
								&& h.i < h.scripts.length) {
							h.start = e;
							this.RestartAfter(MathJax.Callback
									.Delay(this.processUpdateDelay))
						}
					}
				} catch (f) {
				...
				if (h.scripts.length && this.config.showProcessingMessages) {
					MathJax.Message.Set([ "TypesetMath",
							"Typesetting math: %1%%", 100 ], 0);
			...
			processMessage : function(d, b) {
				...
				var c = (b === "Output" ? [ "TypesetMath",
						"Typesetting math: %1%%" ] : [ "ProcessMath",
						"Processing math: %1%%" ]);
			...
			processError : function(b, c, a) {
				...
				return MathJax.Callback.After([ "process" + a, this, c ],
						b.restart)
			},
			...
			elementCallback : function(c, f) {
				if (f == null
						&& (c instanceof Array || typeof c === "function")) {
					try {
						MathJax.Callback(c);
						f = c;
						c = null
		...
		MathJax.Hub.Startup = {
			...
			Config : function() {
				...
				if (this.params.config) {
					...
					for (var b = 0, a = c.length; b < a; b++) {
						...
						this.queue.Push([ "Require", MathJax.Ajax,
								this.URL("config", c[b]) ])
					}
				}
				this.queue
						.Push([ "Config", MathJax.Hub, MathJax.AuthorConfig ]);
			...
			ConfigDelay : function() {
				var a = this.params.delayStartupUntil
			...
			ConfigBlocks : function() {
				...
				for (var d = 0, a = c.length; d < a; d++) {
					...
					if (e.match(/^text\/x-mathjax-config(;.*)?$/)
			...
			Cookie : function() {
				return this.queue
						.Push(
								...
								[ "Get", MathJax.HTML.Cookie, "menu",
										MathJax.Hub.config.menuSettings ],
								[
										function(d) {
											if (d.menuSettings.locale) {
												MathJax.Localization
														.resetLocale(d.menuSettings.locale)
											}
											...
											if (d.menuSettings.CHTMLpreview
													&& !MathJax.Extension["CHTML-preview"]) {
												MathJax.Hub.config.extensions
														.push("CHTML-preview.js")
											}
										}, MathJax.Hub.config ], [ "Post",
										this.signal, "End Cookie" ])
			},
			Styles : function() {
				return this.queue.Push([ "Post", this.signal, "Begin Styles" ],
						[ "loadArray", this, MathJax.Hub.config.styleSheets,
								"config" ], [ "Styles", MathJax.Ajax,
								MathJax.Hub.config.styles ], [ "Post",
								this.signal, "End Styles" ])
			},
			Jax : function() {
				var f = MathJax.Hub.config, c = MathJax.Hub.outputJax;
				for (var g = 0, b = f.jax.length, d = 0; g < b; g++) {
					var e = f.jax[g].substr(7);
					if (f.jax[g].substr(0, 7) === "output/"
							&& c.order[e] == null) {
						c.order[e] = d;
						d++
					}
				...
				return a.Push([ "Post", this.signal, "Begin Jax" ], [
						"loadArray", this, f.jax, "jax", "config.js" ], [
						"Post", this.signal, "End Jax" ])
			},
			Extensions : function() {
				var a = MathJax.Callback.Queue();
				return a.Push([ "Post", this.signal, "Begin Extensions" ], [
						"loadArray", this, MathJax.Hub.config.extensions,
						"extensions" ],
			...
			MenuZoom : function() {
				if (MathJax.Hub.config.showMathMenu) {
					if (!MathJax.Extension.MathMenu) {
						setTimeout(function() {
							MathJax.Callback.Queue([ "Require", MathJax.Ajax,
									"[MathJax]/extensions/MathMenu.js", {} ], [
									"loadDomain", MathJax.Localization,
									"MathMenu" ])
						}, 1000)
					} else {
						setTimeout(MathJax.Callback([ "loadDomain",
								MathJax.Localization, "MathMenu" ]), 1000)
					}
					if (!MathJax.Extension.MathZoom) {
						setTimeout(MathJax.Callback([ "Require", MathJax.Ajax,
								"[MathJax]/extensions/MathZoom.js", {} ]), 2000)
					}
				}
			},
			onLoad : function() {
			...
			Typeset : function(a, b) {
				...
				return this.queue.Push(
						[ "Post", this.signal, "Begin Typeset" ], [ "Typeset",
								MathJax.Hub, a, b ], [ "Post", this.signal,
								"End Typeset" ])
			},
			URL : function(b, a) {
				if (!a.match(/^([a-z]+:\/\/|\[|\/)/)) {
			...
			var g = MathJax.Object.Subclass({
				...
				Augment : function(k, j) {
					...
					if (k != null) {
						...
						if (k.toString !== i.prototype.toString
								&& k.toString !== {}.toString) {
				...
				Translate : function(h, i) {
					throw Error(this.directory + "/" + this.JAXFILE
				...
				loadComplete : function(i) {
					if (i === "config.js") {
						...
						h.Push(c.Register.StartupHook("End Config", {}), [
								"Post", c.Startup.signal,
								this.id + " Jax Config" ], [ "Config", this ],
								[ "Post", c.Startup.signal,
										this.id + " Jax Require" ], [
										function(j) {
											return MathJax.Hub.Startup
													.loadArray(j.require,
															this.directory)
										}, this ], [
										function(j, k) {
											return MathJax.Hub.Startup
													.loadArray(j.extensions,
															"extensions/" + k)
										}, this.config || {}, this.id ], [
										"Post", c.Startup.signal,
										this.id + " Jax Startup" ], [
					...
					.Subclass(
							{
								...
								preProcess : function(j) {
									var i, h = this.directory + "/"
											+ this.JAXFILE;
									this.constructor.prototype.preProcess = function(
											k) {
										if (!i.called) {
											return i
										}
								...
								Register : function(i) {
									...
									this.require.push(b.ElementJax.directory
											+ "/" + (i.split(/\//)[1]) + "/"
											+ this.JAXFILE)
								},
								Remove : function(h) {
								...
								extensionDir : g.extensionDir,
								fontDir : e + (b.isPacked ? "" : "/..")
										+ "/fonts",
								imageDir : e + (b.isPacked ? "" : "/..")
										+ "/images"
							});
		...
		(function(n) {
			...
			var d = h.replace(/^Mozilla\/(\d+\.)+\d+ /, "").replace(
					...
					.Select({
						...
						Firefox : function(j) {
							...
							j.isMobile = (navigator.appVersion
									.match(/Android/i) != null
									|| h.match(/ Fennec\//) != null || h
									.match(/Mobile/) != null)
						},
						Opera : function(i) {
						...
						MSIE : function(j) {
							...
							MathJax.HTML.setScriptBug = !j.isIE9
									|| document.documentMode < 9;
							MathJax.Hub.msieHTMLCollectionBug = (document.documentMode < 9);
							if (document.documentMode < 10
									&& !r.params.NoMathPlayer) {
								try {
								...
								try {
									if (j.hasMathPlayer) {
										...
										if (document.readyState
											...
											document
													.write('<?import namespace="m" implementation="#MathPlayer">');
											j.mpImported = true
										}
									} else {
										document.namespaces.add("mjx_IE_fix",
			...
			if (g.AuthorConfig
					&& typeof g.AuthorConfig.AuthorInit === "function") {
				g.AuthorConfig.AuthorInit()
			}
			c.queue = g.Callback.Queue();
			c.queue.Push([ "Post", r.signal, "Begin" ], [ "Config", r ], [
					"Cookie", r ], [ "Styles", r ], [ "Message", r ],
					function() {
						var i = g.Callback.Queue(r.Jax(), r.Extensions());
						return i.Push({})
					}, [ "Menu", r ], r.onLoad(), function() {
						MathJax.isReady = true
					}, [ "Typeset", r ], [ "Hash", r ], [ "MenuZoom", r ], [
							"Post", r.signal, "End" ])
		})("MathJax")
	}
};
